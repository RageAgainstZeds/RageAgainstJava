...
public abstract class AbstractDAO<K, T> {
    ...
    public List<T> findById(Class<T> cls, int id) {
    ...
    public void update(T t, String collName, String val) {
        try {
            Field[] fields = t.getClass().getDeclaredFields();
            Field coll = null;
            Field id = null;
            ...
            for (Field f : fields) {
                if (f.getName().equalsIgnoreCase(collName)) {
                    coll = f;
            ...
            if (coll == null)
                throw new RuntimeException("No such field to update");

            String sql = "UPDATE " + table + " SET " + coll.getName() +
                    " = \"" + val + "\"" + " WHERE " + id.getName() +
                    " = \"" + id.get(t) + "\"";
